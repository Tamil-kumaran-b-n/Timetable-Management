...
from database.database import add_subject, get_all_subjects, update_subject, delete_subject, search_subjects
...
class SubjectsWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(['ID', 'Subject Code', 'Subject Name', 'Department', 'Semester'])
    ...
    def add_subject_record(self):
        ...
        success = add_subject(subject_code=code, subject_name=name, department=dept, semester=sem)
        if success:
    ...
    def update_subject_record(self):
        ...
        dept = self.dept_entry.text().strip()
        sem = self.sem_combo.currentText().strip()
        success = update_subject(record_id=self.selected_id, subject_code=code, subject_name=name, department=dept, semester=sem)
        if success:
    ...
    def delete_subject_record(self):
        if not self.selected_id:
            ...
            return
        confirm = QMessageBox.question(self, 'Confirm Delete', f"Are you sure you want to delete '{self.name_entry.text().strip()}' ({self.code_entry.text().strip()})?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
