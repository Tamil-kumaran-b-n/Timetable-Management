...
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QGridLayout, QLabel, QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QHeaderView, QMessageBox, QFrame, QScrollArea, QAbstractItemView
from database.database import add_classroom, get_all_classrooms, update_classroom, delete_classroom, search_classrooms
...
class ClassroomsWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(['ID', 'Room Number', 'Room Name'])
    ...
    def add_classroom_record(self):
        ...
        success = add_classroom(room_number=rnum, room_name=rname, building='', room_type='', capacity=0)
        if success:
    ...
    def update_classroom_record(self):
        ...
        rnum = self.room_number_entry.text().strip()
        rname = self.room_name_entry.text().strip() or ''
        success = update_classroom(record_id=self.selected_id, room_number=rnum, room_name=rname, building='', room_type='', capacity=0)
        if success:
    ...
    def delete_classroom_record(self):
        if not self.selected_id:
            ...
            return
        confirm = QMessageBox.question(self, 'Confirm Delete', f"Are you sure you want to delete classroom '{self.room_number_entry.text().strip()}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
