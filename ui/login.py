...
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QCheckBox, QComboBox, QMessageBox, QFrame, QScrollArea, QSizePolicy
from database.database import authenticate_user, add_user, get_all_subjects, get_app_setting
...
class LoginWindow(QMainWindow):
    ...
    def show_login_form(self):
        ...
        self.show_pwd_check.toggled.connect(lambda checked: self.password_entry.setEchoMode(QLineEdit.EchoMode.Normal if checked else QLineEdit.EchoMode.Password))
    ...
    def login(self):
        ...
        if user:
            ...
            QMessageBox.information(self, 'Success', f'Welcome, {display_name}!')
        ...
        if self.subjects_data:
            for s in self.subjects_data:
                ...
                if selected_subject_str.startswith(code) and dept:
                    department = dept
                    break
        success, msg = add_user(username=username, password=password, role='Faculty', full_name=fullname if fullname else username, department=department)
        if not success:
            QMessageBox.critical(self, 'Registration Failed', msg)
            return
        QMessageBox.information(self, 'Faculty Account Created', f"Faculty account '{username}' registered successfully!\n\nLogging in to your Faculty Portal...")
        user = authenticate_user(username, password)
        if self.on_login_success:
            try:
                self.on_login_success(self, user)
