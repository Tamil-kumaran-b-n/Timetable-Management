...
class Dashboard(QMainWindow):
    ...
    def show_page(self, page, add_history=True):
        ...
        titles = {'home': 'Home', 'faculty': 'Faculty', 'subjects': 'Subjects', 'classrooms': 'Classrooms', 'classes': 'Classes', 'assignments': 'Workload', 'generate': 'Generate', 'view_timetable': 'Timetable', 'settings': 'Settings'}
    ...
    def build_home_page(self):
        ...
        qa_layout.addWidget(self.create_action_card('Generate Timetable', 'Create conflict-free class timetables.', 'Generate', lambda: self.show_page('generate')))
        qa_layout.addWidget(self.create_action_card('Faculty', 'Manage faculty members and hours.', 'Open', lambda: self.show_page('faculty')))
        qa_layout.addWidget(self.create_action_card('Subjects', 'Manage subjects, credits, and labs.', 'Open', lambda: self.show_page('subjects')))
    ...
    def refresh_stats(self):
        ...
        if hasattr(self, 'lbl_recent_status'):
            if t_count > 0:
                self.lbl_recent_status.setText('Timetables generated. View or export from the Timetable page.')
            else:
    ...
    def confirm_logout(self):
        confirm = QMessageBox.question(self, 'Log Out', 'Are you sure you want to log out?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
